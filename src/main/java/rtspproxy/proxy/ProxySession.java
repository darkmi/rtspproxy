...
public class ProxySession {
  ...
  private static Map<String, ProxySession> serverSessionIds =
      new ConcurrentHashMap<String, ProxySession>();

  /** Map IDs for RTSP session with clients to ProxySession objects. */
  private static Map<String, ProxySession> clientSessionIds =
      new ConcurrentHashMap<String, ProxySession>();

  /**
   * Retrieve the ProxySession associated with the given session ID used by the client.
   * 
  ...
  /**
   * Retrieve the ProxySession associated with the given session ID used by the server.
   * 
   * @param serverSessionId a string containing the RTSP session ID
   * @return the associated ProxySession or null if not found
   */
  ...
  /**
   * This is the session ID generated by the proxy and used for the communication with the client.
   */
  private String clientSessionId = null;

  /**
   * This is the session ID assigned by the server. RTSP messages with the server must use this ID.
   */
  ...
  /**
   * Construct a new ProxySession. The session ID that will be used when communicating with the
   ...
   * 
   * @param clientSessionId a string containing the session id
   */
  ...
  public synchronized void close() {
    ...
    if (clientSessionId != null) clientSessionIds.remove(clientSessionId);
    if (serverSessionId != null) serverSessionIds.remove(serverSessionId);
  }
