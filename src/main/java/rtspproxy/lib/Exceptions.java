 ...
 * 
 * $URL:
 * http://svn.berlios.de/svnroot/repos/rtspproxy/tags/3.0-ALPHA2/src/main/java/rtspproxy/lib/Exceptions
 * .java $
 */
...
public class Exceptions {
  ...
  /**
   * Print a stack trace of the throwable in the log appender. The trace is only appended if the log
  ...
  public static synchronized void logStackTrace(Throwable throwable) {
    ...
    if (cause != null) sb = getStackTraceAsCause(sb, cause, trace);
  ...
  private static synchronized StringBuilder getStackTraceAsCause(StringBuilder sb, Throwable cause,
    ...
    for (int i = 0; i <= m; i++)
      sb.append("\tat " + trace[i] + CRLF);
    if (framesInCommon != 0) sb.append("\t... " + framesInCommon + " more" + CRLF);

    // Recurse if we have a cause
    Throwable ourCause = cause.getCause();
    if (ourCause != null) sb = getStackTraceAsCause(sb, cause, trace);
