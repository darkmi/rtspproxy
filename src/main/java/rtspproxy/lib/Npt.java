 ...
 * svn+ssh://merlimat@svn.berlios.de/svnroot/
 * repos/rtspproxy/trunk/src/main/java/rtspproxy/lib/PortManager.java $
 */
...
public class Npt {
   ...
   * @return a new Npt object or <code>null</code> if the description is not valid
   */
    ...
    if (m1.matches()) {
      ...
      if ("now".equals(sStart)) {
        ...
        try {
          ...
          if (sEnd.length() != 0) end = Double.parseDouble(sEnd);
  ...
  /**
   * @return Returns true if the timeing is <i>now</i> and so it represents a live event.
   */
