...
public abstract class RtspFilters implements IoFilterChainBuilder {
  ...
  protected void addAuthenticationFilter(IoFilterChain chain) {
    boolean enableAuthenticationFilter =
        Config.getBoolean("proxy.filter.authentication.enable", false);

    if (enableAuthenticationFilter) {
      if (authenticationFilter == null) authenticationFilter = new AuthenticationFilter();
