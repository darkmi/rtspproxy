...
public class RequestUrlRewritingImpl extends IoFilterAdapter {
   ...
   */
  private static final Logger logger = Logger.getLogger(RequestUrlRewritingImpl.class);
   ...
   * 
   * TODO: This may become obsolete if moving to OSGi bundles TODO: Make filter parametrizeable.
   * Could be done by moving from properties to XML config file.
   */
   ...
   * 
   * @see
   * org.apache.mina.common.IoFilterAdapter#messageReceived(org.apache.mina.common.IoFilter.NextFilter
  ...
  private void rewriteUrlHeader(String headerName, RtspResponse resp) {
    ...
    if (oldHeader != null) {
      logger.debug("old content " + headerName + " header value: " + oldHeader);

      try {
        URL header = this.filter.rewriteResponseHeaderUrl(new URL(oldHeader));

        if (header != null) {
          logger.debug("changed header " + headerName + " to " + header);
