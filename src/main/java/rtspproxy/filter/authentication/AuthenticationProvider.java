 ...
 * /authentication/AuthenticationProvider.java $
 */
...
public interface AuthenticationProvider {
  ...
  /**
   * Called once at service startup. Should be used to initialize the provider.
   * 
   * @throws Exception
   */
  ...
  /**
   * Called every time that there is the need to verify the identity of a user.
   * 
   * @param credentials User credentials (username and password)
   * @return true if the user succesfull authenticate with the given username and password. false if
   *         user is not present or the password is wrong.
   */
