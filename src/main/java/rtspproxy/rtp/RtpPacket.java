 ...
 * 
 * $URL:
 * http://svn.berlios.de/svnroot/repos/rtspproxy/tags/3.0-ALPHA2/src/main/java/rtspproxy/rtp/RtpPacket
 * .java $
 */
...
/**
 * This class wraps a RTP packet providing method to convert from and to a {@link IoBuffer}.
 * <p>
 ...
 * present only when inserted by a mixer.
 * 
 * @author Matteo Merli
 */
public class RtpPacket implements Packet {
  ...
  /**
   * Padding flag. If the padding bit is set, the packet contains one or more additional padding
   * octets at the end which are not part of the payload. The last octet of the padding contains a
   * count of how many padding octets should be ignored, including itself. Padding may be needed by
  ...
  /**
   * Extension Flag. If the extension bit is set, the fixed header MUST be followed by exactly one
   * header extension, with a format defined in Section 5.3.1 of the RFC.
   */
   ...
   * additional marker bits or specify that there is no marker bit by changing the number of bits in
   ...
   * formats. Additional payload type codes MAY be defined dynamically through non-RTP means (see
   * Section 3).
   * <p>
   * A set of default mappings for audio and video is specified in the companion RFC 3551 [1]. An
   * RTP source MAY change the payload type during a session, but this field SHOULD NOT be used for
   ...
   * receiver to detect packet loss and to restore packet sequence. The initial value of the
   * sequence number SHOULD be random (unpredictable) to make known-plaintext attacks on encryption
   * more difficult, even if the source itself does not encrypt according to the method in Section
   * 9.1, because the packets may flow through a translator that does.
   */
  private UnsignedShort sequence;

  /**
   * The timestamp reflects the sampling instant of the first octet in the RTP data packet. The
   * sampling instant MUST be derived from a clock that increments monotonically and linearly in
   * time to allow synchronization and jitter calculations (see Section 6.4.1).
   */
  private UnsignedInt timestamp;

  /**
   * The SSRC field identifies the synchronization source. This identifier SHOULD be chosen
   * randomly, with the intent that no two synchronization sources within the same RTP session will
  ...
  /**
   * @param csrcCount The csrcCount to set.
   */
