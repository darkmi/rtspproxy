...
/**
 * A MediaDescription identifies the set of medias that may be received on a specific port or set of
 * ports. It includes:
 * 
 * a mediaType (e.g., audio, video, etc.) a port number (or set of ports) a protocol to be used
 * (e.g., RTP/AVP) a set of media formats which correspond to Attributes associated with the media
 * description.
 * 
...
public interface MediaDescription extends Serializable, Cloneable {
   ...
   * type + '/' + subType
   * <p>
   * The subType portion is computed in the following fashion. RTP/AVP the subType is returned as
   * the codec name. This will either be extracted from the rtpmap attribute or computed. other the
   * protocol is returned as the subType.
   * 
   * If the protocol is RTP/AVP and the rtpmap attribute for a codec is absent, then the codec name
   * will be computed in the following fashion. String indexed in table SdpConstants.avpTypeNames if
   * the value is an int greater than or equal to 0 and less than AVP_DEFINED_STATIC_MAX, and has
   * been assigned a value. SdpConstant.RESERVED if the value is an int greater than or equal to 0
   * and less than AVP_DEFINED_STATIC_MAX, and has not been assigned a value. SdpConstant.UNASSIGNED
   * An int greater than or equal to AVP_DEFINED_STATIC_MAX and less than AVP_DYNAMIC_MIN -
   * currently unassigned. SdpConstant.DYNAMIC Any int less than 0 or greater than or equal to
   * AVP_DYNAMIC_MIN
   * 
