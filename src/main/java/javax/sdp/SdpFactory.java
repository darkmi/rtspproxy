...
public class SdpFactory extends Object
{
	...
	/**
	 * Obtain an instance of an SdpFactory. This static method returns a factory
	 * instance. Once an application has obtained a reference to an SdpFactory
	 * it can use the factory to configure and obtain parser instances and to
	 * create SDP objects.
	 * 
	 * @throws SdpException
	 ...
	 * v=0 o=this.createOrigin ("user", InetAddress.getLocalHost().toString());
	 * s=- t=0 0
	 * 
	 * @throws SdpException SdpException, - if there is a problem constructing
	 ...
	 * within the string parameter. Note: unknown field types should not cause
	 * exceptions.
	 * 
	 * @param s s - the sdp message that is to be parsed.
	 * @throws SdpParseException SdpParseException - if there is a problem
	 *         parsing the String.
	 ...
	 * @param numPorts number of ports used for this media stream
	 * @param transport transport type, eg "RTP/AVP"
	 * @param staticRtpAvpTypes list of static RTP/AVP media payload types which
	 *        should be specified by the returned MediaDescription throws
	 *        IllegalArgumentException if passed an invalid RTP/AVP payload type
	 * @throws IllegalArgumentException
	 * @throws SdpException
	 * @return MediaDescription
	 */
	public MediaDescription createMediaDescription( String media, int port, int numPorts,
			String transport, int[] staticRtpAvpTypes ) throws IllegalArgumentException,
			SdpException
	{
	 ...
	 * equivalent to createConnection("IN", "IP4", addr);
	 * 
	 * @param addr connection address
	 ...
	 * @param offset the adjustment in number of seconds relative to the start
	 *        time of the SessionDescription with which this object is
	 *        associated.
	 ...
	 * 
	 * @param args
	 * @throws SdpException public static void main(String[] args) throws
	 *         SdpException { }
	 */

	/**
	 * Returns a collection of Strings containing session description. This
	 * method is no longer supported, as the semantics are unclear and the
	 * original implementation caused a memory leak.
	 * 
	 ...
	 * SdpException { String sdpFields = "v=0\r\n"+ "o=phone 1057165447541
	 * 1057165447541 IN IP4 123.4.566.156\r\n"+ "s=-\r\n" + "c=IN IP4
	 * 123.4.566.156\r\n"+ "t=0 0\r\n" + "m=data 3011 tcp OSA\r\n"; SdpFactory
	 * sdpFactory = new SdpFactory(); SessionDescription sessionDescription =
	 * sdpFactory.createSessionDescription(sdpFields);
	 * System.out.println("sessionDescription = " + sessionDescription); Vector
	 * mediaDescriptions = sessionDescription.getMediaDescriptions(false); for
	 * (int i = 0; i < mediaDescriptions.size(); i++) { MediaDescription m =
	 * (MediaDescription) mediaDescriptions.elementAt(i); System.out.println("m = " +
	 * m.toString()); Media media = m.getMedia(); Vector formats =
	 * media.getMediaFormats(false); System.out.println("formats = " + formats); } }
	 */
