...
/**
 * The SdpFactory enables applications to encode and decode SDP messages. The SdpFactory can be used
 * to construct a SessionDescription object programmatically. The SdpFactory can also be used to
 * construct a SessionDescription based on the contents of a String. Acknowledgement: Bugs reported
 * by Brian J. Collins <bjcollins@rockwellcollins.com>. and by Majdi Abuelbassal
 * <majdi.abuelbassal@bbumail.com>. Please refer to IETF RFC 2327 for a description of SDP.
 * 
 * @author Olivier Deruelle <olivier.deruelle@nist.gov>
 * @author M. Ranganathan <mranga@nist.gov> <a href="{@docRoot} /uncopyright.html">This code is in
 *         the public domain.</a>
 * @version 1.0
 */
public class SdpFactory extends Object {
  ...
  /**
   * Obtain an instance of an SdpFactory. This static method returns a factory instance. Once an
   * application has obtained a reference to an SdpFactory it can use the factory to configure and
   * obtain parser instances and to create SDP objects.
   * 
   * @throws SdpException
  ...
  public Media createMedia(String media, int port, int numPorts, String transport,
      Vector<String> staticRtpAvpTypes) throws SdpException {
   ...
   */
  public MediaDescription createMediaDescription(String media, int port, int numPorts,
      String transport, int[] staticRtpAvpTypes) throws IllegalArgumentException, SdpException {
   ...
   */
  public MediaDescription createMediaDescription(String media, int port, int numPorts,
      String transport, String[] formats) {
    MediaDescriptionImpl mediaDescriptionImpl = new MediaDescriptionImpl();
    try {
   ...
   * 
   * @param t the Time that the time description applies to. Returns TimeDescription object with the
  ...
  /**
   * Returns a String containing the computed form for a multi-connection address. Parameters: addr
   * - connection address ttl - time to live (TTL) for multicast addresses numAddrs - number of
   * addresses used by the connection Returns: a String containing the computed form for a
   ...
   * @return Connection
   */
  public Connection createConnection(String netType, String addrType, String addr, int ttl,
  ...
  /**
   * Returns a Connection object with the specified properties and no TTL and a default number of
   * addresses (1).
   * 
   * @param netType network type, eg "IN" for "Internet"
   * @param addrType address type, eg "IP4" for IPv4 type addresses
   ...
   * @return Connection
   */
  public Connection createConnection(String netType, String addrType, String addr)
      throws SdpException {
    ConnectionField connectionImpl = new ConnectionField();
  ...
  /**
   * Returns a Connection object with the specified properties and a network and address type of
   * "IN" and "IP4" respectively.
   * 
   * @param addr connection address
   * @param ttl time to live (TTL) for multicast addresses
   * @param numAddrs number of addresses used by the connection
  ...
  /**
   * Returns a collection of Strings containing session description. This method is no longer
   * supported, as the semantics are unclear and the original implementation caused a memory leak.
   * 
  ...
  /**
   * public static void main(String[] args) throws SdpParseException , SdpException { String
   * sdpFields = "v=0\r\n"+ "o=phone 1057165447541 1057165447541 IN IP4
   * 123.4.566.156\r\n"+ "s=-\r\n" + "c=IN IP4 123.4.566.156\r\n"+ "t=0 0\r\n" + "m=data 3011 tcp
   * OSA\r\n"; SdpFactory sdpFactory = new SdpFactory(); SessionDescription sessionDescription =
   * sdpFactory.createSessionDescription(sdpFields); System.out.println("sessionDescription = " +
   * sessionDescription); Vector mediaDescriptions = sessionDescription.getMediaDescriptions(false);
   * for (int i = 0; i < mediaDescriptions.size(); i++) { MediaDescription m = (MediaDescription)
   * mediaDescriptions.elementAt(i); System.out.println("m = " + m.toString()); Media media =
   * m.getMedia(); Vector formats = media.getMediaFormats(false); System.out.println("formats = " +
   * formats); } }
   */
