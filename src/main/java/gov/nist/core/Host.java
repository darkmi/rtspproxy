...
public class Host extends GenericObject {
  ...
  public Host(String hostName) throws IllegalArgumentException {
    if (hostName == null) throw new IllegalArgumentException("null host name");
    this.hostname = hostName;
    if (isIPv6Address(hostName)) this.addressType = IPV6ADDRESS;
  ...
  public String encode() {
    if (addressType == IPV6ADDRESS && !isIPv6Reference(hostname)) return "[" + hostname + "]";
    return hostname;
  }

  /**
   * Compare for equality of hosts. Host names are compared by textual equality. No dns lookup is
   * performed.
   * 
  ...
  /**
   * Get the inet address from this host. Caches the inet address returned from dns lookup to avoid
   * lookup delays.
   * 
...
/*
 * $Log: Host.java,v $ Revision 1.5 2005/04/16 20:38:42 dmuresan Canonical clone() implementations
 * for the GenericObject and GenericObjectList hierarchies
 * 
