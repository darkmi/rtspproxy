...
/**
 * The base class from which all the other classes in the sipheader, sdpfields and sipmessage
 * packages are extended. Provides a few utility funcitons such as indentation and pretty printing
 * that all other classes benifit from.
 * 
...
public abstract class GenericObject implements Serializable, Cloneable {
  ...
  protected static final String[] immutableClassNames = {"String", "Character", "Boolean", "Byte",
  ...
  /**
   * Set the pattern matcher. To match on the field of a sip message, set the match expression in
   * the match template and invoke the match function. This useful because SIP headers and
   * parameters may appear in different orders and are not necessarily in canonical form. This makes
   * it hard to write a pattern matcher that relies on regular expressions alone. Thus we rely on
   * the following strategy i.e. To do pattern matching on an incoming message, first parse it, and
   * then construct a match template, filling in the fields that you want to match. The rules for
   * matching are: A null object matches wild card - that is a match template of null matches any
   * parsed SIP object. To match with any subfield, set the match template on a template object of
   * the same type and invoke the match interface. Regular expressions matching implements the
   * gov.nist.sip.Match interface that can be done using the Jakarta regexp package for example.
   * package included herein. This can be used to implement the Match interface <a
   * href=http://www.apache.org> See the APACHE website for documents </a>
   * 
   */
  public void setMatcher(Match matchExpression) {
    if (matchExpression == null) throw new IllegalArgumentException("null arg!");
  ...
  /**
   * Clones the given object. If the object is a wrapped type, an array, a GenericObject or a
   * GenericObjectList, it is cast to the appropriate type and the clone() method is invoked. Else
   * if the object implements Cloneable, reflection is used to discover and invoke the public
  ...
  public static Object makeClone(Object obj) {
    ...
    if (immutableClasses.contains(c))
      ...
      } catch (SecurityException ex) {} catch (IllegalArgumentException ex) {
        InternalErrorHandler.handleException(ex);
      } catch (IllegalAccessException ex) {} catch (InvocationTargetException ex) {} catch (NoSuchMethodException ex) {}
    }
    return clone_obj;
  }
  ...
  /**
   * Recursively override the fields of this object with the fields of a new object. This is useful
   * when you want to genrate a template and override the fields of an incoming SIPMessage with
   * another SIP message that you have already generated.
   * 
   * @param mergeObject is the replacement object. The override obect must be of the same class as
   *        this object. Set any fields that you do not want to override as null in the mergeOject
   *        object.
   */
   ...
   * two protocl Objects.
   * 
   * @param other the match pattern to test against. The match object has to be of the same type
   *        (class). Primitive types and non-sip fields that are non null are matched for equality.
   *        Null in any field matches anything. Some book-keeping fields are ignored when making the
   *        comparison.
   */
   ...
   *        A replacement will occur if a portion of the structure is found with matching encoded
   *        text (a substring if matchSubstring is true) as objectText and with the same class as
   *        replacement.
   */
   ...
   *        portion of the structure is found with a match of the encoded text with objectText and
   *        with the same class as replacement.
   * @param matchSubstring is true if we want to match objectText as a substring of the encoded
   *        target text. (i.e. an object is a candidate for replacement if objectText is a substring
   *        of candidate.encode() && candidate.class.equals(replacement.class) otherwise the match
   *        test is an equality test.)
   */
  ...
  /**
   * Do a recursive find and replace of objects pointed to by this object based on regular
   * expression pattern matching.
   * 
   * @since v1.0
   * @param regexp regular expression for the object we want to find. This is generated using a
   *        regular expression matching package such as the apache regexp package.
   * @param replacement object we want to replace this portion with. A replacement will occur if a
   *        portion of the structure is found with a match of the encoded text with objectText and
   *        with the same class as replacement.
   */
  public void replace(Match regexp, GenericObjectList replacement) throws IllegalArgumentException {
   ...
   * 
   * @param regexp is the match expression (i.e. implementation of the Match interface) for the
   *        object that we want to replace.
   * @param replacement is the object that we want to replace it with. A replacement will occur if a
   *        portion of the structure is found that matches according to the given regexp and if the
   *        class of the replaced field matches the replacement.
   */
  public void replace(Match regexp, GenericObject replacement) throws IllegalArgumentException {
    if (regexp == null || replacement == null) {
      throw new IllegalArgumentException("null argument!");
    }
    Class replacementClass = replacement.getClass();
...
/*
 * $Log: GenericObject.java,v $ Revision 1.11 2005/04/16 20:38:42 dmuresan Canonical clone()
 * implementations for the GenericObject and GenericObjectList hierarchies
 * 
 * Revision 1.10 2005/04/16 20:37:07 dmuresan GenericObject and GenericObjectList implement
 * Cloneable.
 * 
 * Revision 1.9 2005/04/16 20:36:12 dmuresan Optimized GenericObject.makeClone().
 * 
 * Revision 1.8 2005/04/04 10:43:04 dmuresan Strings and wrapped types no longer cloned in
 * GenericObject.makeClone()
 * 
 * Revision 1.7 2005/04/04 09:51:37 dmuresan Optimized getIndentation() implementations (previously
 * used String concatenation in a loop).
 * 
 * Revision 1.6 2005/04/04 08:27:02 dmuresan Optimized GenericObject.sprint() for primitive types.
 * 
 * Revision 1.5 2004/01/22 14:23:45 mranga Reviewed by: mranga Fixed some minor formatting issues.
 * 
 * Revision 1.4 2004/01/22 13:26:27 sverker Issue number: Obtained from: Submitted by: sverker
 * Reviewed by: mranga
 * 
 * Major reformat of code to conform with style guide. Resolved compiler and javadoc warnings. Added
 * CVS tags.
 * 
 * CVS: ---------------------------------------------------------------------- CVS: Issue number:
 * CVS: If this change addresses one or more issues, CVS: then enter the issue number(s) here. CVS:
 * Obtained from: CVS: If this change has been taken from another system, CVS: then name the system
 * in this line, otherwise delete it. CVS: Submitted by: CVS: If this code has been contributed to
 * the project by someone else; i.e., CVS: they sent us a patch or a set of diffs, then include
 * their name/email CVS: address here. If this is your work then delete this line. CVS: Reviewed by:
 * CVS: If we are doing pre-commit code reviews and someone else has CVS: reviewed your changes,
 * include their name(s) here. CVS: If you have not had it reviewed then delete this line.
 */
