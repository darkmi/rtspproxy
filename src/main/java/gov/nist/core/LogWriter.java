...
public class LogWriter {
  ...
  /**
   * Flag to indicate that logging is enabled. This needs to be static and public in order to
   * globally turn logging on or off.
   */
  ...
  public void logThrowable(Throwable throwable) {
    if (needsLogging) {
      ...
      PrintWriter pw = new PrintWriter(sw);
      checkLogFile();
      if (printWriter != null) throwable.printStackTrace(pw);
      pw.close();
      println(sw.toString());
    }
  }
...
/*
 * $Log: LogWriter.java,v $ Revision 1.9 2004/09/26 14:48:02 mranga Submitted by: John Martin
 * Reviewed by: mranga
 * 
 * Remove unnecssary synchronization.
 * 
 * Revision 1.8 2004/09/01 02:04:33 xoba Issue number: no particular issue number.
 * 
 ...
 * javadocs hyperlink to jre api documentation). since top-level 'docs' directory already contains
 * cvs-controlled files, i redirected output of javadocs to their own separate directories, which
 * are 'cleaned' along with 'clean' target. also created other javadoc which just outputs javax.*
 * classes for those wishing to develop sip applications without reference to nist.gov.*.
 * 
 ...
 * 
 * Revision 1.7 2004/04/19 18:23:48 mranga Reviewed by: mranga Fixed the tck (reset factories before
 * getting the TI factories)
 * 
 * 
