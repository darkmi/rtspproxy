...
/**
 * Implements a homogenous consistent linked list. All the objects in the linked list must derive
 * from the same root class. This is a useful constraint to place on our code as this property is
 * invariant.The list is created with the superclass which can be specified as either a class name
 * or a Class.
 * 
...
public abstract class GenericObjectList extends LinkedList implements Serializable, Cloneable {
  // Useful constants.
   ...
   * tail end of this list. A type check is done before concatenation.
   * 
   * @param <var> topFlag </var> set to true to add items to top of list else add them to the tail
   *        end of the list.
   */
  ...
  public String debugDump() {
    ...
    while (obj != null) {
      sprint("[");
      try {
        if (Class.forName(PackageNames.CORE_PACKAGE + ".GenericObjectList").isAssignableFrom(
            obj.getClass())) {
          sprint(((GenericObjectList) obj).debugDump(this.indentation));
        } else if (Class.forName(PackageNames.CORE_PACKAGE + ".GenericObject").isAssignableFrom(
            obj.getClass())) {
  ...
  public boolean add(Object obj) {
    if (myClass == null) {
      ...
      if (!myClass.isAssignableFrom(newclass)) {
        throw new IllegalArgumentException("Class mismatch list insertion  " + listName + " "
  ...
  /**
   * Type checked add operation. All objects in this list are assignable from a common superclass.
   * If the class is already set, then the new one is just compared with the existing class objects.
   * Otherwise the first object that is added determines the class of the objects in the list.
   */
   ...
   * @param replacementObject object to replace the target with ( in case a target is found).
   * @param matchSubstring boolean that indicates whether to flag a match when objectText is a
   *        substring of a candidate object's encoded text.
   */
    ...
    while (listIterator.hasNext()) {
      ...
      if (GenericObject.isMySubclass(obj.getClass())) {
        ...
        if (gobj.getClass().equals(replacementObject.getClass())) {
          if ((!matchSubstring) && gobj.encode().compareTo(objectText) == 0) {
            // Found the object that we want,
            ll.add(obj);
          } else if (matchSubstring && gobj.encode().indexOf(objectText) >= 0) {
            ll.add(obj);
          } else {
            gobj.replace(objectText, replacementObject, matchSubstring);
          }
        }
   ...
   *        substring of a candidate object's encoded text.
   */
  public void replace(String objectText, GenericObjectList replacementObject, boolean matchSubstring)
      throws IllegalArgumentException {
    if (objectText == null || replacementObject == null) {
      throw new IllegalArgumentException("null argument");
    }
   ...
   * Do a find and replace of objects in this list.
   * 
   * @param regexp regular expression to match with the canonical text we want to replace.
   * @param replacementObject object to replace the target with (in case a target is found).
   */
   ...
   * Do a find and replace of objects in this list.
   * 
   * @param regexp match regular expression of the object to find. this is generated using the
   *        org.apache.regexp package.
   * @param replacementObject object to replace the target with ( in case a target is found).
   */
  ...
  /**
   * Do a merge of the GenericObjects contained in this list with the GenericObjects in the
   * mergeList. Note that this does an inplace modification of the given list. This does an object
   * by object merge of the given objects.
   * 
   * @param mergeList is the list of Generic objects that we want to do an object by object merge
   *        with. Note that no new objects are added to this list.
   * 
   */
  ...
  /**
   * Match with a template (return true if we have a superset of the given template. This can be
   * used for partial match (template matching of SIP objects). Note -- this implementation is not
   * unnecessarily efficient :-)
   * 
...
/*
 * $Log: GenericObjectList.java,v $ Revision 1.9 2005/04/16 20:38:42 dmuresan Canonical clone()
 * implementations for the GenericObject and GenericObjectList hierarchies
 * 
 * Revision 1.8 2005/04/16 20:37:07 dmuresan GenericObject and GenericObjectList implement
 * Cloneable.
 * 
 * Revision 1.7 2005/04/04 09:51:38 dmuresan Optimized getIndentation() implementations (previously
 * used String concatenation in a loop).
 * 
 * Revision 1.6 2004/01/22 14:23:45 mranga Reviewed by: mranga Fixed some minor formatting issues.
 * 
 * Revision 1.5 2004/01/22 13:26:27 sverker Issue number: Obtained from: Submitted by: sverker
 * Reviewed by: mranga
 * 
