...
public class LexerCore extends StringTokenizer {
  ...
  public Token match(int tok) throws ParseException {
    ...
    if (tok > START && tok < END) {
      ...
      if (tok == DIGIT) {
        ...
        this.currentMatch = new Token();
        this.currentMatch.tokenValue = new StringBuffer().append(next).toString();
        this.currentMatch.tokenType = tok;
        consume(1);
  ...
  public boolean startsId() {
    try {
      ...
      return (isAlpha(nextChar) || isDigit(nextChar) || nextChar == '_' || nextChar == '+'
          || nextChar == '-' || nextChar == '!' || nextChar == '`' || nextChar == '\''
          || nextChar == '~' || nextChar == '%' // bug fix by Bruno Konik
          || nextChar == '.' || nextChar == '*');
    } catch (ParseException ex) {
      return false;
    }
  ...
  public String ttoken() {
    ...
    try {
      while (hasMoreChars()) {
        ...
        if (isAlpha(nextChar) || isDigit(nextChar) || nextChar == '_' || nextChar == '+'
            || nextChar == '-' || nextChar == '!' || nextChar == '`' || nextChar == '\''
            || nextChar == '~' || nextChar == '%' // bug fix by Bruno Konik
            || nextChar == '.' || nextChar == '*') {
          consume(1);
...
/*
 * $Log: LexerCore.java,v $ Revision 1.11 2004/10/21 14:57:16 mranga Reviewed by: mranga
 * 
 * Fixed origin field parser for sdp.
 * 
 * Revision 1.10 2004/08/10 23:15:50 mranga Reviewed by: mranga improved error message
 * 
 * Revision 1.9 2004/03/10 03:37:25 mranga Submitted by: Ben Evans Reviewed by: mranga Bug in
 * parsing of via header when there is a number of comma separated headers.
 * 
 * Revision 1.8 2004/02/06 20:15:55 mranga Submitted by: Bruno Konik Reviewed by: mranga Fixed
 * character sets for parsing of urls and methods.
 * 
 * Revision 1.7 2004/01/22 14:23:45 mranga Reviewed by: mranga Fixed some minor formatting issues.
 * 
 * Revision 1.6 2004/01/22 13:26:27 sverker Issue number: Obtained from: Submitted by: sverker
 * Reviewed by: mranga
 * 
