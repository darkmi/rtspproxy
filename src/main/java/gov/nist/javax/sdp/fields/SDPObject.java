...
public abstract class SDPObject extends GenericObject implements SDPFieldNames {
  ...
  /**
   * Generic introspection based printing funciton for this package. Because we use introspection,
   * this must reside in this package and we cannot use the superclass defintion (even though it is
   * virtually identical.
   */
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      try {
        ...
        if (fieldType.isPrimitive()) {
          ...
          } else if (hisObj != null && GenericObject.isMySubclass(myObj.getClass())
   ...
   *        A replacement will occur if a portion of the structure is found with the same encoded
   *        text as objectText and with the same class as replacement.
   */
  ...
  public void replace(String objectText, GenericObject replacement, boolean matchSubstring)
    ...
    for (int i = 0; i < fields.length; i++) {
      Field f = fields[i];
      Class fieldType = f.getType();
      if (!getClassFromName(CORE_PACKAGE + ".GenericObject").isAssignableFrom(fieldType)
          && !getClassFromName(CORE_PACKAGE + ".GenericObjectList").isAssignableFrom(fieldType)) {
        continue;
      ...
      try {
        ...
        } else if (getClassFromName(CORE_PACKAGE + ".GenericObject").isAssignableFrom(fieldType)) {
          GenericObject g = (GenericObject) f.get(this);
          g.replace(objectText, replacement, matchSubstring);
        } else if (getClassFromName(CORE_PACKAGE + ".GenericObjectList")
   ...
   *        portion of the structure is found with the matching encoded text as objectText and with
   *        the same class as the replacement.
   * @param matchSubstring is true if we want to match the encoded text of a candidate object as a
   *        substring of the encoded target text. ( match occurs is objectText is a substring of the
   *        encoded text of an object with the same class as replacement.)
   */
  @SuppressWarnings("unchecked")
...
/*
 * $Log: SDPObject.java,v $ Revision 1.3 2005/04/16 20:38:44 dmuresan Canonical clone()
 * implementations for the GenericObject and GenericObjectList hierarchies Revision 1.2 2004/01/22
 * 13:26:27 sverker Issue number: Obtained from: Submitted by: sverker Reviewed by: mranga Major
 * reformat of code to conform with style guide. Resolved compiler and javadoc warnings. Added CVS
 * tags. CVS: ---------------------------------------------------------------------- CVS: Issue
 * number: CVS: If this change addresses one or more issues, CVS: then enter the issue number(s)
 * here. CVS: Obtained from: CVS: If this change has been taken from another system, CVS: then name
 * the system in this line, otherwise delete it. CVS: Submitted by: CVS: If this code has been
 * contributed to the project by someone else; i.e., CVS: they sent us a patch or a set of diffs,
 * then include their name/email CVS: address here. If this is your work then delete this line. CVS:
 * Reviewed by: CVS: If we are doing pre-commit code reviews and someone else has CVS: reviewed your
 * changes, include their name(s) here. CVS: If you have not had it reviewed then delete this line.
 */
