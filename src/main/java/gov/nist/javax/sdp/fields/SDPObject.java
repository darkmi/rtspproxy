...
public abstract class SDPObject extends GenericObject implements SDPFieldNames
{
	...
	protected static final String SDPFIELDS_PACKAGE = PackageNames.SDP_PACKAGE
			+ ".fields";
	...
	/**
	 * Generic introspection based printing funciton for this package. Because
	 * we use introspection, this must reside in this package and we cannot use
	 * the superclass defintion (even though it is virtually identical.
	 */
		...
		for ( int i = 0; i < fields.length; i++ ) {
			...
			try {
				...
				if ( fieldType.isPrimitive() ) {
					...
					if ( Class.forName( CORE_PACKAGE + ".GenericObject" ).isAssignableFrom(
							fieldType ) ) {
						if ( f.get( this ) != null ) {
						...
						if ( Class.forName( CORE_PACKAGE + ".GenericObjectList" ).isAssignableFrom(
								fieldType ) ) {
							if ( f.get( this ) != null ) {
	...
	/**
	 * An introspection based predicate matching using a template object. Allows
	 * for partial match of two protocl Objects.
	 * 
	 * @param other the match pattern to test against. The match object has to
	 *            be of the same type (class). Primitive types and non-sip
	 *            fields that are non null are matched for equality. Null in any
	 *            field matches anything. Some book-keeping fields are ignored
	...
	/**
	 * Recursively override the fields of this object with the fields of a new
	 * object. This is useful when you want to genrate a template and override
	 * the fields of an incoming SIPMessage with another SIP message that you
	 * have already generated.
	 * 
	 * @param mergeObject is the replacement object. The override obect must be
	 *            of the same class as this object. Set any fields that you do
	 *            not want to override as null in the mergeOject object.
	 */
	 ...
	 * 
	 * @since v1.0
	 * @param objectText is the canonical string representation of the object
	 *            that we want to replace.
	 * @param replacement is the object that we want to replace it with.
	 * @param matchSubstring a boolean which tells if we should match a
	 *            substring of the target object A replacement will occur if a
	 *            portion of the structure is found with the same encoded text
	 *            as objectText and with the same class as replacement.
	 */
	@SuppressWarnings("unchecked")
	public void replace( String objectText, GenericObject replacement,
			boolean matchSubstring ) throws IllegalArgumentException
	{
		...
		for ( int i = 0; i < fields.length; i++ ) {
			Field f = fields[i];
			Class fieldType = f.getType();
			if ( !getClassFromName( CORE_PACKAGE + ".GenericObject" ).isAssignableFrom(
					fieldType )
					&& !getClassFromName( CORE_PACKAGE + ".GenericObjectList" ).isAssignableFrom(
							fieldType ) ) {
				continue;
			} else
			...
			try {
				if ( fieldType.equals( replacementClass ) ) {
					...
					if ( GenericObjectList.isMySubclass( replacementClass ) ) {
						...
						if ( getClassFromName( CORE_PACKAGE + ".GenericObject" ).isAssignableFrom(
							...
							if ( getClassFromName( CORE_PACKAGE + ".GenericObjectList" ).isAssignableFrom(
									fieldType ) ) {
	 ...
	 * 
	 * @since v1.0
	 * @param objectText Canonical string representation of the portion we want
	 *            to replace.
	 * @param replacement object we want to replace this portion with. A
	 *            replacement will occur if a portion of the structure is found
	 *            with the matching encoded text as objectText and with the same
	 *            class as the replacement.
	 * @param matchSubstring is true if we want to match the encoded text of a
	 *            candidate object as a substring of the encoded target text. (
	 *            match occurs is objectText is a substring of the encoded text
	 *            of an object with the same class as replacement.)
	 */
	@SuppressWarnings("unchecked")
	public void replace( String objectText, GenericObjectList replacement,
			boolean matchSubstring ) throws IllegalArgumentException
	{
	 ...
	 * 
	 * @since v1.0
	 * @param regexp regular expression for the object we want to find. This is
	 *            generated using a regular expression matching package such as
	 *            the apache regexp package.
	 * @param replacement object we want to replace this portion with. A
	 *            replacement will occur if a portion of the structure is found
	 *            with a match of the encoded text with objectText and with the
	 *            same class as replacement.
	 */
	@SuppressWarnings("unchecked")
	...
	/**
	 * Do a find and replace of objects based on regular expression matching of
	 * fields.
	 * 
	 * @param regexp is the match expression (i.e. implementation of the Match
	 *            interface) for the object that we want to replace.
	 * @param replacement is the object that we want to replace it with. A
	 *            replacement will occur if a portion of the structure is found
...
/*
 * $Log: SDPObject.java,v $ Revision 1.3 2005/04/16 20:38:44 dmuresan Canonical
 * clone() implementations for the GenericObject and GenericObjectList
 * hierarchies Revision 1.2 2004/01/22 13:26:27 sverker Issue number: Obtained
 * from: Submitted by: sverker Reviewed by: mranga Major reformat of code to
 * conform with style guide. Resolved compiler and javadoc warnings. Added CVS
 * tags. CVS:
 * ---------------------------------------------------------------------- CVS:
