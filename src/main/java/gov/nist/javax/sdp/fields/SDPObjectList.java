...
public class SDPObjectList extends GenericObjectList
{
	 ...
	 * the given objects.
	 * 
	 * @param mergeList is the list of Generic objects that we want to do an
	 *            object by object merge with. Note that no new objects are
	 *            added to this list.
	 */
	 ...
	 * @param replacementObject object to replace the target with ( in case a
	 *            target is found).
	 * @param matchSubstring boolean that indicates whether to flag a match when
	 *            objectText is a substring of a candidate object's encoded
	 *            text.
	 */
	@SuppressWarnings("unchecked")
	public void replace( String objectText, GenericObject replacementObject,
			...
			if ( GenericObject.isMySubclass( obj.getClass() ) ) {
				...
				if ( gobj.getClass().equals( replacementObject.getClass() ) ) {
					if ( ( !matchSubstring )
						...
						if ( matchSubstring && gobj.encode().indexOf( objectText ) >= 0 ) {
							ll.add( obj );
						} else {
				...
				if ( GenericObjectList.isMySubclass( obj.getClass() ) ) {
	 ...
	 */
	@SuppressWarnings("unchecked")
	public void replace( String objectText, GenericObjectList replacementObject,
			boolean matchSubstring ) throws IllegalArgumentException
	{
		if ( objectText == null || replacementObject == null ) {
			throw new IllegalArgumentException( "null argument" );
...
/*
 * $Log: SDPObjectList.java,v $ Revision 1.5 2005/04/16 20:38:45 dmuresan
 * Canonical clone() implementations for the GenericObject and GenericObjectList
 ...
 * javax.sdp. Revision 1.3 2004/01/22 13:26:27 sverker Issue number: Obtained
