...
public class ZoneField extends SDPField implements javax.sdp.TimeZoneAdjustment
{
	...
	/**
	 * Returns a Hashtable of adjustment times, where: key = Date. This is the
	 * equivalent of the decimal NTP time value. value = Int Adjustment. This is
	 * a relative time value in seconds.
	 * 
	 * @param create to set
	...
	/**
	 * Sets the Hashtable of adjustment times, where: key = Date. This is the
	 * equivalent of the decimal NTP time value. value = Int Adjustment. This is
	 * a relative time value in seconds.
	 * 
	 ...
	 * Typed time is formatted as an integer followed by a unit character. The
	 * unit indicates an appropriate multiplier for the integer. The following
	 * unit types are allowed. d - days (86400 seconds) h - hours (3600 seconds)
	 * m - minutes (60 seconds) s - seconds ( 1 seconds)
	 * 
	 * @param typedTime typedTime - if set true, the start and stop times will
	 *        be output in an optimal typed time format; if false, the times
	 *        will be output as integers.
	 */
	...
	/**
	 * Returns whether the field will be output as a typed time or a integer
	 * value. Typed time is formatted as an integer followed by a unit
	 * character. The unit indicates an appropriate multiplier for the integer.
	 * The following unit types are allowed. d - days (86400 seconds) h - hours
	 * (3600 seconds) m - minutes (60 seconds) s - seconds ( 1 seconds)
	 * 
	 * @return true, if the field will be output as a typed time; false, if as
	 *         an integer value.
	 */
...
/*
 * $Log: ZoneField.java,v $ Revision 1.5 2005/04/16 20:38:45 dmuresan Canonical
 * clone() implementations for the GenericObject and GenericObjectList
 * hierarchies Revision 1.4 2005/04/04 10:01:28 dmuresan Used StringBuffer
 * instead of String += for concatenation in various encode() methods in
 * javax.sdp. Revision 1.3 2004/01/22 13:26:28 sverker Issue number: Obtained
