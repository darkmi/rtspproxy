...
public class RepeatField extends SDPField implements javax.sdp.RepeatTime {
  ...
  public void setRepeatInterval(int repeatInterval) throws SdpException {
    if (repeatInterval < 0)
      ...
      if (this.repeatInterval == null) this.repeatInterval = new TypedTime();
  ...
  public void setActiveDuration(int activeDuration) throws SdpException {
    if (activeDuration < 0)
      ...
      if (this.activeDuration == null) this.activeDuration = new TypedTime();
      this.activeDuration.setTime(activeDuration);
    }
  }

  /**
   * Returns the list of offsets. These are relative to the start-time given in the Time object (t=
  ...
  /**
   * Set the list of offsets. These are relative to the start-time given in the Time object (t=
   * field) with which this RepeatTime is associated.
   * 
   ...
   * 
   * The following unit types are allowed. d - days (86400 seconds) h - hours (3600 seconds) m -
   * minutes (60 seconds) s - seconds ( 1 seconds)
   * 
   * @throws SdpParseException
   ...
   * minutes (60 seconds) s - seconds ( 1 seconds)
   * 
   * @param typedTime typedTime - if set true, the start and stop times will be output in an optimal
   *        typed time format; if false, the times will be output as integers.
   */
  ...
  public String encode() {
    ...
    retval.append(REPEAT_FIELD).append(repeatInterval.encode()).append(Separators.SP)
  ...
  public Object clone() {
    ...
    if (this.offsets != null) retval.offsets = (SDPObjectList) this.offsets.clone();
    return retval;
  }

}
/*
 * $Log: RepeatField.java,v $ Revision 1.4 2005/04/16 20:38:44 dmuresan Canonical clone()
 * implementations for the GenericObject and GenericObjectList hierarchies
 * 
 * Revision 1.3 2005/04/04 10:01:27 dmuresan Used StringBuffer instead of String += for
 * concatenation in various encode() methods in javax.sdp.
 * 
 * Revision 1.2 2004/01/22 13:26:27 sverker Issue number: Obtained from: Submitted by: sverker
 * Reviewed by: mranga
 * 
