...
public class MediaDescriptionImpl implements javax.sdp.MediaDescription {
  ...
  public String encode() {
    ...
    if (mediaField != null) retval.append(mediaField.encode());

    if (informationField != null) retval.append(informationField.encode());

    if (connectionField != null) retval.append(connectionField.encode());
    ...
    if (keyField != null) retval.append(keyField.encode());
  ...
  public void setInfo(Info i) throws SdpException {
    if (i == null) throw new SdpException("The info is null");
  ...
  public void setBandwidths(Vector<BandWidth> bandwidths) throws SdpException {
    if (bandwidths == null) throw new SdpException("The vector bandwidths is null");
  ...
  public int getBandwidth(String name) throws SdpParseException {
    ...
    if (name == null) throw new NullPointerException("null parameter");
    if (bandwidthFields == null)
      return -1;
    else {
      for (int i = 0; i < bandwidthFields.size(); i++) {
        ...
        if (type != null && type.equals(name)) return bandwidthField.getBandwidth();
      }
      return -1;
    }
  }
  ...
  public void setBandwidth(String name, int value) throws SdpException {
    if (name == null)
      ...
      for (int i = 0; i < bandwidthFields.size(); i++) {
        BandwidthField bandwidthField = (BandwidthField) bandwidthFields.elementAt(i);
        String type = bandwidthField.getBwtype();
        if (type != null && type.equals(name)) bandwidthField.setBandwidth(value);
      }
  ...
  public void removeBandwidth(String name) {
    if (name == null) {
      ...
      if (i < bandwidthFields.size()) bandwidthFields.removeElementAt(i);
    }
  }
   ...
   * 
   * @param create specifies whether to return null or a new empty Vector in case no attributes
   *        exists for this Description
  ...
  /**
   * Returns a Vector containing a string indicating the MIME type for each of the codecs in this
   * description. A MIME value is computed for each codec in the media description. The MIME type is
   * computed in the following fashion: The type is the mediaType from the media field. The subType
   * is determined by the protocol. The result is computed as the string of the form: type + '/' +
   * subType The subType portion is computed in the following fashion. RTP/AVP the subType is
   * returned as the codec name. This will either be extracted from the rtpmap attribute or
   * computed. other the protocol is returned as the subType. If the protocol is RTP/AVP and the
   * rtpmap attribute for a codec is absent, then the codec name will be computed in the following
   * fashion. String indexed in table SdpConstants.avpTypeNames if the value is an int greater than
   * or equal to 0 and less than AVP_DEFINED_STATIC_MAX, and has been assigned a value.
   * SdpConstant.RESERVED if the value is an int greater than or equal to 0 and less than
   * AVP_DEFINED_STATIC_MAX, and has not been assigned a value. SdpConstant.UNASSIGNED An int
   * greater than or equal to AVP_DEFINED_STATIC_MAX and less than AVP_DYNAMIC_MIN - currently
   * unassigned. SdpConstant.DYNAMIC Any int less than 0 or greater than or equal to AVP_DYNAMIC_MIN
   * 
   * @throws SdpException if there is a problem extracting the parameters.
   * @return a Vector containing a string indicating the MIME type for each of the codecs in this
   *         description
   */
   ...
   * A parameter string is computed for each codec. The parameter string is computed in the
   * following fashion. The rate is extracted from the rtpmap or static data. The number of channels
   * is extracted from the rtpmap or static data. The ptime is extracted from the ptime attribute.
   * The maxptime is extracted from the maxptime attribute. Any additional parameters are extracted
   * from the ftmp attribute.
   * 
   * @throws SdpException if there is a problem extracting the parameters.
   ...
   * 
   * @param payloadNames a Vector of String - each one the name of a dynamic payload to be added
   *        (usually an integer larger than SdpConstants.AVP_DYNAMIC_MIN).
   * @param payloadValues a Vector of String - each contains the value describing the correlated
   *        dynamic payloads to be added
...
/*
 * $Log: MediaDescriptionImpl.java,v $ Revision 1.3 2004/12/13 02:39:34 mranga Reviewed by: mranga
 * javadoc bug Revision 1.2 2004/01/22 13:26:27 sverker Issue number: Obtained from: Submitted by:
 * sverker Reviewed by: mranga Major reformat of code to conform with style guide. Resolved compiler
 * and javadoc warnings. Added CVS tags. CVS:
