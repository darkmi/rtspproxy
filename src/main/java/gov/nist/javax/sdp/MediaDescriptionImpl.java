...
public class MediaDescriptionImpl implements javax.sdp.MediaDescription
{
	 ...
	 * the codecs in this description. A MIME value is computed for each codec
	 * in the media description. The MIME type is computed in the following
	 * fashion: The type is the mediaType from the media field. The subType is
	 * determined by the protocol. The result is computed as the string of the
	 * form: type + '/' + subType The subType portion is computed in the
	 * following fashion. RTP/AVP the subType is returned as the codec name.
	 * This will either be extracted from the rtpmap attribute or computed.
	 * other the protocol is returned as the subType. If the protocol is RTP/AVP
	 * and the rtpmap attribute for a codec is absent, then the codec name will
	 * be computed in the following fashion. String indexed in table
	 * SdpConstants.avpTypeNames if the value is an int greater than or equal to
	 * 0 and less than AVP_DEFINED_STATIC_MAX, and has been assigned a value.
	 * SdpConstant.RESERVED if the value is an int greater than or equal to 0
	 * and less than AVP_DEFINED_STATIC_MAX, and has not been assigned a value.
	 * SdpConstant.UNASSIGNED An int greater than or equal to
	 * AVP_DEFINED_STATIC_MAX and less than AVP_DYNAMIC_MIN - currently
	 ...
	 * in this description. A parameter string is computed for each codec. The
	 * parameter string is computed in the following fashion. The rate is
	 * extracted from the rtpmap or static data. The number of channels is
	 * extracted from the rtpmap or static data. The ptime is extracted from the
	 * ptime attribute. The maxptime is extracted from the maxptime attribute.
	 ...
	 * @param payloadValues a Vector of String - each contains the value
	 *            describing the correlated dynamic payloads to be added
	 * @throws SdpException if either vector is null or empty. if the vector
	 *             sizes are unequal.
	 */
...
/*
 * $Log: MediaDescriptionImpl.java,v $ Revision 1.3 2004/12/13 02:39:34 mranga
 * Reviewed by: mranga javadoc bug Revision 1.2 2004/01/22 13:26:27 sverker
